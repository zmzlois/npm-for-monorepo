...
export default function Page() {
  ...
  const renderIndex = pageNumber === 0 ? pageNumber : pageNumber * 20;
  ...
  return (
    <div className="flex flex-col gap-4">
      <ul className="container mx-auto flex flex-col items-start px-20 py-5">
        {" "}
        <TransitionRoot isOpen={isOpen}>
          {result.slice(renderIndex, renderIndex + 20).map((item, index) => (
            <TransitionItem key={index} isOpen={isOpen}>
              <li
                key={index}
                id={`item-${index}`}
